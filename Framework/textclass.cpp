...
bool TextClass::SetNumberOfObject(int InObjectNum, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char objString[16];
	bool result;
	...
	// Convert the cpu integer to string format.
	_itoa_s(InObjectNum, tempString, 10);
...
bool TextClass::SetNumberOfPolygon(int InPolygonNum, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char polyString[16];
	bool result;
	...
	// Convert the cpu integer to string format.
	_itoa_s(InPolygonNum, tempString, 10);
...
bool TextClass::SetScreenResoiution(int width, int height, ID3D11DeviceContext* deviceContext)
{
	char tempStringWidth[16];
	char tempStringHeight[16];
	char screenString[16];
	bool result;


	// Convert the cpu integer to string format.
	_itoa_s(width, tempStringWidth, 10);
	_itoa_s(height, tempStringHeight, 10);

	// Setup the cpu string.
	strcpy_s(screenString, "");
	strcat_s(screenString, tempStringWidth);
	strcat_s(screenString, " x ");
	strcat_s(screenString, tempStringHeight);


	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, screenString, 20, 100, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
