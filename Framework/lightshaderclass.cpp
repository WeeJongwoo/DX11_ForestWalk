...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount,
	...
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower, XMFLOAT4 pointDiffuseColor[], XMFLOAT4 lightPosition[], bool bIsRotate)
{
	bool result;
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture,
		lightDirection, ambientColor, diffuseColor, cameraPosition, specularColor, specularPower, pointDiffuseColor, lightPosition);
	if (!result)
	{
		return false;
	}
	...
	RenderShader(deviceContext, indexCount, instanceCount, bIsRotate);
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* fileName)
{
	...
	//--------------------------------------------------------------------------------------------------------------
	ID3D10Blob* vertexRotateShaderBuffer;

	// Initialize the pointers this function will use to null.
	errorMessage = 0;
	vertexShaderBuffer = 0;
	pixelShaderBuffer = 0;
	vertexRotateShaderBuffer = 0;
	...
	result = D3DCompileFromFile(fileName, NULL, NULL, "LightVertexRotateShader", "vs_5_0",
		D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexRotateShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
	...
	result = device->CreateVertexShader(vertexRotateShaderBuffer->GetBufferPointer(), vertexRotateShaderBuffer->GetBufferSize(), NULL, &m_vertexRotationShader);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = device->CreateInputLayout(polygonLayout, numElements, vertexRotateShaderBuffer->GetBufferPointer(), vertexRotateShaderBuffer->GetBufferSize(),
		&m_layout);
	if (FAILED(result))
	{
		return false;
	...
	vertexRotateShaderBuffer->Release();
	vertexRotateShaderBuffer = 0;
...
void LightShaderClass::ShutdownShader()
{
	...
	if (m_vertexRotationShader)
	{
		m_vertexRotationShader->Release();
		m_vertexRotationShader = 0;
	}
...
void LightShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount, bool bIsRotate)
{
	...
	if(!bIsRotate)
		...
		deviceContext->VSSetShader(m_vertexRotationShader, NULL, 0);
